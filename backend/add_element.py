...
def process_and_embed_file(file_path, description=None, force=False):
    ...
    Skips the file if its hash hasn't changed in the DB (unless force=True).
    """
    ...
    # For images and PDFs, derive a text description so the embedding lands in
    # the same text space as user queries. Without this, image bytes go through
    # a text-only embedder and search quality is poor.
    if not description:
